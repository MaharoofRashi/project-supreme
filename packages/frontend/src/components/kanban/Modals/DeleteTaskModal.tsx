...
const DeleteTaskModal = ({ onClose, onOpen }: DeleteTaskModalProps) => {
    ...
    return (
        <div
            onClick={handleClick}
            className="bg-black/50 z-20 top-0 left-0  w-screen fixed h-screen flex items-center justify-center"
        >
            <div
                ref={bodyRef}
                className="w-85.5 md:w-480 p-6 md:p-8 md:pb-10 bg-bgLight dark:bg-bgDark rounded-md"
            >
                <h2 className="text-secondary text-lg leading-tight">
                    Delete this task?
                </h2>
