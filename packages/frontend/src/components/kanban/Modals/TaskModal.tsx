...
const TaskModal = forwardRef<HTMLDialogElement, TaskModalProps>(
    ({ onClose, openConfirmation }, ref) => {
        ...
        const completedSubTasks = useStore((store) =>
            store.currentTask.subtasks.filter((sub) => sub.isCompleted),
        );
        ...
        const handleClick = (
            ...
            if (
                !openMenu &&
                modalBody.current &&
                !modalBody.current.contains(target)
            ) {
                onClose();
            }
        };
        ...
        return (
            ...
            >
                <div
                    ref={modalBody}
                    className="w-85.5 md:w-480 p-6 pb-8 md:p-8 bg-white dark:bg-darkGrey max-w-[480px] rounded-md flex flex-col gap-y-6"
                >
