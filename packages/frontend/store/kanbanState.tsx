...
import { getDatas } from '../src/Utils/func';
import { immer } from 'zustand/middleware/immer';
import { ITab, ITasks } from '../src/Utils/Types';
        ...
        immer((set) => ({
            ...
            updateBoard: (board, currentId) =>
                set((state) => {
                    const index = state.data.findIndex(
                        (b) => b.id === currentId,
                    ); // Utilisation de l'ID pour trouver le tableau
                    if (index !== -1) {
            ...
            changeStatusByClick: (status) =>
                set((state) => {
                    ...
                    const task = {
                        ...state.currentBoard.columns[currentColIndex].tasks[
                            taskIndex
                        ],
                        status,
                    };
                    state.currentBoard.columns[currentColIndex].tasks.splice(
                        taskIndex,
                        1,
                    );
            ...
            deleteTask: () =>
                set((state) => {
                    const indexBoard = state.data.findIndex(
                        (board) => board.id === state.currentBoard.id,
                    );
                    if (indexBoard === -1) {
                        console.error('Board not found');
                    ...
                    const indexCol = state.currentBoard.columns.findIndex(
                        (col) =>
                            col.tasks.some(
                                (task) => task.id === state.currentTask.id,
                            ),
                    ...
                    const indexTask = state.currentBoard.columns[
                        indexCol
                    ].tasks.findIndex(
                        (task) => task.id === state.currentTask.id,
                    );
                    ...
                    state.currentBoard.columns[indexCol].tasks.splice(
                        indexTask,
                        1,
                    );
                    state.data[indexBoard].columns[indexCol].tasks.splice(
                        indexTask,
                        1,
                    );
                    state.currentTask = {
            ...
            updateTask: (task) =>
                set((state) => {
                    ...
                    state.currentBoard.columns[indexCol].tasks[indexTask] =
                        task;
                    state.data[indexBoard].columns[indexCol].tasks[indexTask] =
                        task;
                    state.currentTask = task;
                }),
            moveTask: (currColId, currTaskIndex, destColId, destTaskIndex) =>
                set((state) => {
                    const indexBoard = state.data.findIndex(
                        (board) => board.id === state.currentBoard.id,
                    );
                    ...
                    const colSrcIndex = state.currentBoard.columns.findIndex(
                        (col) => col.id === currColId,
                    );
                    if (colSrcIndex === -1) {
                        console.error('Source Column not found');
                        return;
                    }
                    const nextColIndex = state.currentBoard.columns.findIndex(
                    ...
                    const task =
                        state.currentBoard.columns[colSrcIndex].tasks[
                            currTaskIndex
                        ];
                    state.currentBoard.columns[colSrcIndex].tasks.splice(
                        currTaskIndex,
                        1,
                    );
                    state.currentBoard.columns[nextColIndex].tasks.splice(
                        destTaskIndex,
                        0,
                        task,
                    );
                    state.data[indexBoard].columns[colSrcIndex].tasks.splice(
                        currTaskIndex,
                        1,
                    );
                    state.data[indexBoard].columns[nextColIndex].tasks.splice(
                        destTaskIndex,
                        0,
                        task,
                    );
